...
export default function TrackScreen() {
  ...
  async function handleStop() {
    ...
    try {
      ...
      Alert.alert('Run saved', '', [{ text: 'OK', onPress: () => router.back() }]);
    } catch (err: any) {
      Alert.alert('Failed to save', err.response?.data?.error || 'Something went wrong');
    } finally {
      setIsSaving(false);
    }
  }
    ...
    return (
      <SafeAreaView style={styles.maximizedContainer}>
        <View style={styles.headerRow}>
          ...
          <Pressable style={styles.iconButtonPlain} onPress={() => setIsMaximized(false)}>
        ...
        <View style={styles.mainStatsSection}>
          <View style={styles.primaryStat}>
            <Text style={styles.primaryValue}>{formatDistance(totalDistance)}</Text>
          ...
          <View style={styles.primaryStat}>
            <Text style={styles.primaryValue}>
              {totalDistance > 0 ? formatPace(totalDistance, elapsedSeconds) : '--:--'}
            </Text>
        ...
        <View style={styles.actionSection}>
          {!isTracking ? (
            ...
            <Pressable style={styles.stopButton} onPress={handleStop} disabled={isSaving}>
              <Text style={styles.buttonText}>{isSaving ? 'SAVING...' : 'FINISH & SAVE'}</Text>
            </Pressable>
          )}
        </View>
      </SafeAreaView>
      ...
      <SafeAreaView style={styles.overlayContainer} edges={['bottom']}>
        <View style={styles.overlayCard}>
          <View style={styles.cardHeader}>
            ...
            <Pressable style={styles.iconButtonPlain} onPress={() => setIsMaximized(true)}>
          ...
          {!isTracking ? (
            ...
            <Pressable style={styles.stopButton} onPress={handleStop} disabled={isSaving}>
              <Text style={styles.buttonText}>{isSaving ? 'SAVING...' : 'STOP & SAVE'}</Text>
            </Pressable>
          )}
        </View>
      </SafeAreaView>
